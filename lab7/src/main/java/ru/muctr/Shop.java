package ru.muctr; /** Проект Shop содержит 3 класса: Shop - взаимодействует с покупателем,
 * Warehouse - склад
...
public class Shop {
    ...
    public static void main(String[] args){
        ...
        ShopParser shopParser = ShopParser.getInstance();
        Accounting acc = Accounting.getInstance();
        Warehouse wh = new Warehouse();
        DeliveryService dc = new DeliveryService();
        String item;
        ...
        while (true) {
            ...
            if (item.equals("break")){
                ...
                if (item.contains(" ")) {
                    ...
                    if (price.containsKey(item) & wh.quantity.getOrDefault(item, 0) > 0) {
                        shopParser.notifyObservers(new Item(item, price.get(item)));
